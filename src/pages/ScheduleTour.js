...
const TABLE_HEAD = [
  { id: "name", label: "Tour", alignRight: false },
...
function descendingComparator(a, b, orderBy) {
  ...
  export default function ScheduleTour() {
    ...
    const clickAddScheduleTour = async () =>{
      ...
      let link = 'https://be-travel.herokuapp.com/scheduletour/createScheduleTour'
    ...
    useEffect(() => {
      callApi(
        `scheduletour/getAllScheduleTour`,
        "GET"
      ).then((res) => {
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - allScheduleTour.length) : 0;
  
    const filteredScheduleTour = applySortFilter(allScheduleTour, getComparator(order, orderBy), filterName);
      ...
      <Page title="Lịch Trình Chuyến Đi | TRAVEL">
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredScheduleTour
                      ...
                      .map((row) => {
                        const { _id, slot, booked, MFG, EXP, startDate, endDate, tour, status, idTour, deleted} = row;
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell align="left" >{new Date(MFG).toISOString('vi-VN').slice(0, 10)}</TableCell>
                            <TableCell align="left">{new Date(EXP).toISOString('vi-VN').slice(0, 10)}</TableCell>
        ...
        >
          ...
          <Fade in={openAddScheduleTour}>
            ...
            <TextField style={{marginTop: '10px', width: '100%'}} id="outlined-basic" label="Số lượng chỗ" variant="outlined" value={slot} onChange={(event)=>setSlot(event.target.value)}/>
