...
export default function BookTourSchedule() {
  ...
  //lấy danh sách user
  const [allBookTourSchedule, setBookTourSchedule] = useState([]);
  const { id } = useParams();
  useEffect(() => {
    callApi(
      `scheduletour/getBookTourOfScheduleTour?id=${id}`,
      "GET"
    ).then((res) => {
      console.log(res.data.data)
      setBookTourSchedule(res.data.data);
    });
  }, []);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = allBookTourSchedule.map((n) => n._id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - allBookTourSchedule.length) : 0;

  const filteredBookTourSchedule = applySortFilter(allBookTourSchedule, getComparator(order, orderBy), filterName);

  const isUserNotFound = filteredBookTourSchedule.length === 0;


  return (
    <Page title="Các chỗ đã đặt của lịch trình | TravelApp">
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            Tour đã đặt (Số lượng {allBookTourSchedule.length})
          </Typography>
        ...
        <Card>
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={allBookTourSchedule.length}
                  numSelected={selected.length}
                  onRequestSort={handleRequestSort}
                  onSelectAllClick={handleSelectAllClick}
                />
                <TableBody>
                  {filteredBookTourSchedule
          ...
          <TablePagination
            ...
            component="div"
            count={allBookTourSchedule.length}
